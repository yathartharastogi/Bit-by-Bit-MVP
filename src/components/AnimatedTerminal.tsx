...
export default function AnimatedTerminal() {
    ...
    const runAnimation = () => {
      ...
      const typeCmd1 = () => {
        ...
        if (charIdx1 < cmd1.length) {
          ...
          charIdx1++
          setTimeout(typeCmd1, 90 + Math.random() * 50)
        } else {
          setTimeout(() => {
            if (!isMounted) return
            setActiveLine(2)
  ...
  return (
    <div 
      ...
      className="w-full h-full bg-[#030912]/95 border border-border-custom rounded-2xl p-5 font-mono text-xs shadow-2xl relative overflow-hidden group"
    >
      ...
      >
        ...
        <div className="text-white flex items-center flex-wrap">
          ...
          <span>{text1}</span>
          {activeLine === 1 && <span className="inline-block w-1.5 h-4 bg-primary ml-1 animate-pulse" />}
        </div>
        
        <div className="space-y-2 pl-4">
          {outputsVisible >= 1 && (
            <div className="text-emerald-400 flex items-center gap-2 animate-fade-in">
              <span>✓</span><span>Initializing 500+ member network</span>
            </div>
          )}
          {outputsVisible >= 2 && (
            <div className="text-emerald-400 flex items-center gap-2 animate-fade-in">
              <span>✓</span><span>Loading hackathons, workshops...</span>
            </div>
          )}
          {outputsVisible >= 3 && (
            <div className="text-emerald-400 flex items-center gap-2 animate-fade-in">
              <span>✓</span><span>Building real-world projects</span>
            </div>
          )}
          {outputsVisible >= 4 && (
            <div className="text-purple-400 font-semibold flex items-center gap-2 animate-fade-in">
              <span>✓</span><span>Community first. Always.</span>
            </div>
          )}
        </div>
        ...
        {activeLine >= 3 && (
          <div className="text-white flex items-center flex-wrap">
            <span className="text-primary font-bold mr-2 select-none">$ </span>
            <span>{text2}</span>
            {activeLine === 3 && <span className="inline-block w-1.5 h-4 bg-primary ml-1 animate-pulse" />}
            {activeLine === 4 && <span className="inline-block w-1.5 h-4 bg-primary ml-1 animate-pulse" />}
          </div>
        )}
      </div>
    </div>
